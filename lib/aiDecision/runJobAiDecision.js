...
export async function runJobAiDecision({ jobId, job, emitLog, emitSnapshot }) {
  ...
  const promptTextOverride = String(options.promptText || "").trim();
  const promptLabel = String(options.promptLabel || "").trim();
  ...
  emitLog(jobId, "info", "Job gestartet (Ollama KI Prüfung)", {
    ...
    dryRun,
    limit: limit || "ALL",
    promptFile: promptTextOverride ? "(custom)" : promptFile || "(default)",
  ...
  if (promptTextOverride) {
    ...
    promptFileResolved = promptLabel ? `(custom:${promptLabel})` : "(custom)";
  } else {
    try {
